...
底层基于**数组+链表**，默认的容量为**16**,负载因子**0.75**。当数量达到了16×0.75=**12**时触发扩容机制，而扩容的过程涉及到rehash、复制数据等操作，所以建议提前预估HashMap的大小，尽量减少扩容带来的性能损耗。真正存放数据的是`Entry<K, V>[] table`，`Entry`是`HashMap`中的一个静态内部类，有`key、value、next、hash`成员变量。

put过程：判断当前数组是否需要初始化，如果key为空，则put一个空值进去，根据key计算出hashCode，根据hashCode定位出所在桶，如果桶是一个链表则需要遍历判断里面的hashCode、key是否和传入的key相等，如果相等则进行覆盖，并返回原来的值，如果桶是空的，说明当前位置没有数据存入，新增一个Entry对象写入当前位置，调用addEntry时需要判断是否需要扩容，如果需要就进行两倍扩容，并将当前的key重新hash定位，而在createEntry中会将当前位置的桶传入到新建的桶中，如果当前桶有值就会在位置形成链表。

get过程：根据key计算hashCode，定位到具体的桶，判断该位置是否为链表，不是链表就根据key、hashCode是否相等来返回值，是链表就需要遍历key、hashCode相等时返回值。

**HashMap 1.8原理**

当Hash冲突严重时，在桶上形成的链表会越来越长，查询时的效率就会越来越低，因此1.8进行了优化，底层基于**数组+链表+红黑树**。

**SparseArray、ArrayMap对比HashMap**

SparseArray底层基于两个数组，一个存储key，另一个存储value，避免了对key的自动装箱（int转Integer），内部对数据还采取了压缩的方式表示稀疏数组数据，节约内存空间，SparseArray在存储和读取数据时，使用二分查找法，速度快

ArrayMap底层基于两个数组，一个存储hash值，一个2倍大小的依次存储key和value，插入时，根据key的hashCode计算index位置，利用二分查找在对应位置进行插入，出现hash冲突，在index相邻位置插入（开放地址法）。
